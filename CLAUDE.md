...
1. **YOU MUST keep everything working from `file://` with zero network.**
   ...
   The project also deploys to Vercel Hobby as a **second, additive** lane
   (`spec/10-deployment.md`) — that lane may use a manifest, a service worker,
   and clean URLs, all gated behind `LC.env.hosted`, but nothing may be
   written so the `file://` lane starts depending on a network request.
...
**If your change touches a file `sw.js` precaches** (anything under
`assets/`, `content/`, or a lesson page), **bump `CACHE_VERSION` in `sw.js`.**
There is no build step and so no content hashing — that constant is the only
thing that gets a hosted, already-installed client off stale JS.

## Layout

```
index.html             Quest map hub
404.html                Themed error page (hosted lane)
lessons/lesson-NN.html  Thin ~30-line shell
content/lesson-NN.js    All text (he+en), examples, exercises for that lesson
assets/js/              env, early, boot · store, i18n, curriculum, engine, editor,
                        checker, game, lesson, hub
...
assets/img/             PWA icons + OG image, generated by tools/make-icons.mjs
...
| deployment, `vercel.json`, `sw.js`, hosted-only code | `spec/10-deployment.md` |
